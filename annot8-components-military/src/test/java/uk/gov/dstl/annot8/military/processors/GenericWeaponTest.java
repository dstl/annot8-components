...
public class GenericWeaponTest {
  ...
  public void testCreation(){
    ...
    Processor processor = genericWeapon
  ...
  public void testSimple(){
    testSingleMatch("Natalie had a tactical assault rifle hidden in her cupboard.", "tactical assault rifle", "firearm");
  }
  ...
  private void testSingleMatch(String sentence, String match, String subtype){
    ...
    content.getAnnotations().create().withBounds(new SpanBounds(0, sentence.length())).withType(AnnotationTypes.ANNOTATION_TYPE_SENTENCE).save();

    GenericWeapon genericWeapon = new GenericWeapon();
    Processor processor = genericWeapon.createComponent(null, NoSettings.getInstance());
    processor.process(item);

    assertEquals(1, content.getAnnotations().getByType(AnnotationTypes.ANNOTATION_TYPE_WEAPON).count());

    Annotation a = content.getAnnotations().getByType(AnnotationTypes.ANNOTATION_TYPE_WEAPON).findFirst().get();
    assertNotNull(a);
